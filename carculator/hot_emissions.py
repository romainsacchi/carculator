    ...
    def get_emissions_per_powertrain(self, powertrain_type, euro_class):
        ...
        if powertrain_type == "diesel":
            ...
            if euro_class == 5:
                ...
                ) + (0.0011 / average_trip_length)
                # N2O
                em_arr[:, 8] = 0.008 + (0.0074 / average_trip_length)
                # NH3
                em_arr[:, 9] = 0.002 + (1.44e-4 / average_trip_length)
                # Benzene
                em_arr[:, 10] = ne.evaluate(
                    "-8.782e-11 * c ** 3 + 3.059e-08 * c ** 2 - 3.761e-06 * c + 0.0001954"
                ) + (6.66e-5 / average_trip_length)
            ...
            if euro_class == 6.0:
                ...
                # N2O
                em_arr[:, 8] = 0.01 + (0.012 / average_trip_length)
                # NH3
                em_arr[:, 9] = 0.006 + (0.007 / average_trip_length)
                # Benzene
                em_arr[:, 10] = ne.evaluate(
                    "2.525e-10 * c ** 3 - 4.873e-08 * c ** 2 + 2.399e-06 * c + 0.0001"
                ) + (1.43e-4 / average_trip_length)
            ...
            if euro_class == 6.1:
                ...
                ) + (0.00105 / average_trip_length)
                # N2O
                em_arr[:, 8] = 0.01 + (0.01173 / average_trip_length)
                # NH3
                em_arr[:, 9] = 0.006 + (0.0069 / average_trip_length)
                # Benzene
                em_arr[:, 10] = ne.evaluate(
                    "7.713e-11 * c ** 3 - 1.519e-08 * c ** 2 + 9.757e-07 * c + 6.621e-05"
                ) + (1.08e-4 / average_trip_length)
            ...
            if euro_class == 6.3:
                ...
                ) + (9.55e-4 / average_trip_length)

                # N2O
                em_arr[:, 8] = 0.014 + (0.01173 / average_trip_length)
        ...
        if powertrain_type == "CNG":
            ...
            if euro_class == 2:
                ...
                ) + (0.105707355 / average_trip_length)
                # CO
                em_arr[:, 1] = ne.evaluate(
                    "1.573e-05 * c ** 3 - 0.003288 * c ** 2 + 0.2176 * c - 4.045"
                ) + (0.801136017 / average_trip_length)
                # NO
                em_arr[:, 2] = ne.evaluate(
                    "(3.802e-07 * c ** 3 - 4.275e-05 * c ** 2 - 0.001211 * c + 0.2782)"
                ) + (0.141743928 / average_trip_length)

                # PM <= 2.5 um
                em_arr[:, 3] = ne.evaluate(
                    "5.509e-08 * c ** 3 - 9.59e-06 * c ** 2 + 0.0005602 * c - 0.008264"
                ) + (0.007912168 / average_trip_length)
                # CH4
                em_arr[:, 4] = ne.evaluate(
                    "-3.202e-08 * c ** 3 + 9.001e-06 * c ** 2 - 0.000614 * c + 0.04573"
                ) + (0.097250767 / average_trip_length)
                # NMVOC
                em_arr[:, 5] = ne.evaluate(
                    "-1.281e-08 * c ** 3 + 3.6e-06 * c ** 2 - 0.0002456 * c + 0.01829"
                ) + (0.008456588 / average_trip_length)
                # Pb
                em_arr[:, 6] = ne.evaluate(
                ...
                # N2O
                em_arr[:, 8] = 0.002 + (0.000572766 / average_trip_length)
                # NH3
                em_arr[:, 9] = 0.075 + (0.037029848 / average_trip_length)
                # Benzene
                em_arr[:, 10] = ne.evaluate(
            ...
            if euro_class == 3:
                ...
                em_arr[:, 0] = ne.evaluate(
                    "6.91e-08 * c ** 3 - 1.357e-05 * c ** 2 + 0.0009447 * c - 0.01034"
                ) + (0.018378355 / average_trip_length)
                # CO
                em_arr[:, 1] = ne.evaluate(
                    "1.472e-05 * c ** 3 - 0.003084 * c ** 2 + 0.2045 * c - 3.806"
                ) + (0.74397397 / average_trip_length)
                # NOx
                em_arr[:, 2] = ne.evaluate(
                    "(2.075e-07 * c ** 3 - 2.335e-05 * c ** 2 - 0.0006691 * c + 0.1524)"
                ) + (0.125161722 / average_trip_length)

                # PM <= 2.5 um
                em_arr[:, 3] = ne.evaluate(
                    "1.73e-08 * c ** 3 - 2.854e-06 * c ** 2 + 0.0001556 * c - 0.001652"
                ) + (0.003107077 / average_trip_length)
                # CH4
                em_arr[:, 4] = ne.evaluate(
                    "4.215e-08 * c ** 3 - 8.28e-06 * c ** 2 + 0.0005763 * c - 0.006305"
                ) + (0.016908085 / average_trip_length)
                # NMVOC
                em_arr[:, 5] = ne.evaluate(
                    "2.695e-08 * c ** 3 - 5.294e-06 * c ** 2 + 0.0003684 * c - 0.004031"
                ) + (0.001470268 / average_trip_length)
                # Pb
                em_arr[:, 6] = ne.evaluate(
                ...
                # N2O
                em_arr[:, 8] = 2.5e-4 + (0.000572766 / average_trip_length)
                # NH3
                em_arr[:, 9] = 0.03 + (0.037029848 / average_trip_length)
                # Benzene
                em_arr[:, 10] = ne.evaluate(
            ...
            if euro_class == 5:
                ...
                # N2O
                em_arr[:, 8] = 8.5e-4 + (0.000572766 / average_trip_length)
                # NH3
                em_arr[:, 9] = 0.019 + (0.016007606 / average_trip_length)
                # Benzene
                em_arr[:, 10] = ne.evaluate(
            ...
            if euro_class == 6:
                ...
                ) + (1.55e-03 / average_trip_length)
                # NMVOC
                em_arr[:, 5] = ne.evaluate(
                    "9.338e-09 * c ** 3 - 1.872e-06 * c ** 2 + 0.0001175 * c - 9.78e-05"
                ) + (1.74e-03 / average_trip_length)
                # Pb
                em_arr[:, 6] = ne.evaluate(
                ...
                # N2O
                em_arr[:, 8] = 8.5e-4 + (5.73e-04 / average_trip_length)
                # NH3
                em_arr[:, 9] = 0.019 + (1.88e-02 / average_trip_length)
                # Benzene
                em_arr[:, 10] = ne.evaluate(
