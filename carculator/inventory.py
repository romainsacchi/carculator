...
class InventoryCalculation:
    ...
    def __init__(self, array):
        ...
        self.array_inputs = {x:i for i,x in enumerate(list(self.array.parameter.values),0)}
        self.array_powertrains = {x: i for i, x in enumerate(list(self.array.powertrain.values), 0)}
    ...
    def set_inputs_in_A_matrix(self):
        ...
        self.A[self.inputs[('market for glider, passenger car', 'GLO')], -self.number_of_cars :] = \
            self.temp_array[self.array_inputs['glider base mass'],:] / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1


        self.A[self.inputs[('Glider lightweighting', 'GLO')], -self.number_of_cars :] = \
            self.temp_array[self.array_inputs['lightweighting'], :] / self.temp_array[
        ...
        self.A[self.inputs[('market for manual dismantling of used electric passenger car','GLO')], -self.number_of_cars :] = \
            self.temp_array[self.array_inputs['curb mass'], :] * (1 - self.temp_array[self.array_inputs['combustion power share'],:])\
                / 1180 / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1
        ...
        self.A[self.inputs[('market for manual dismantling of used passenger car with internal combustion engine','GLO')], -self.number_of_cars :] =\
            self.temp_array[self.array_inputs['curb mass'],:] * self.temp_array[self.array_inputs['combustion power share'],:]\
                / 1600 / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        # Powertrain components
        self.A[self.inputs[('market for charger, electric passenger car','GLO')], -self.number_of_cars :] =\
            self.temp_array[self.array_inputs['charger mass'],:] / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        self.A[self.inputs[('market for converter, for electric passenger car','GLO')], -self.number_of_cars :] =\
            self.temp_array[self.array_inputs['converter mass'],:] / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        self.A[self.inputs[('market for electric motor, electric passenger car','GLO')], -self.number_of_cars :] =\
            self.temp_array[self.array_inputs['electric engine mass'],:] / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        self.A[self.inputs[('market for inverter, for electric passenger car','GLO')], -self.number_of_cars :] =\
            self.temp_array[self.array_inputs['inverter mass'],:] / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        self.A[self.inputs[('market for power distribution unit, for electric passenger car','GLO')], -self.number_of_cars :] =\
            self.temp_array[self.array_inputs['power distribution unit mass'],:] / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1
        ...
        self.A[self.inputs[('market for used powertrain from electric passenger car, manual dismantling','GLO')], -self.number_of_cars :] =\
            self.temp_array[[self.array_inputs[l] for l in l_elec_pt],:].sum(axis=0) / self.temp_array[self.array_inputs['lifetime kilometers'],:]

        self.A[self.inputs[('market for internal combustion engine, passenger car','GLO')], -self.number_of_cars :] =\
            (self.temp_array[[self.array_inputs[l] for l in ['combustion engine mass','electric engine mass', 'powertrain mass']],:].sum(axis=0))\
                / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        self.A[self.inputs[('Ancillary BoP','GLO')], -self.number_of_cars :] =\
            self.temp_array[self.array_inputs['fuel cell ancillary BoP mass'],:] / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        self.A[self.inputs[('Essential BoP','GLO')], -self.number_of_cars :] =\
            self.temp_array[self.array_inputs['fuel cell essential BoP mass'],:] / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        self.A[self.inputs[('Stack 2020','GLO')], -self.number_of_cars :] =\
            self.temp_array[self.array_inputs['fuel cell stack mass'],:] / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        # Energy storage
        self.A[self.inputs[('Battery BoP','GLO')], -self.number_of_cars :] =\
            (self.temp_array[self.array_inputs['battery BoP mass'],:] * (1 + self.temp_array[self.array_inputs['battery lifetime replacements'],:]))\
                / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1

        self.A[self.inputs[('Battery cell','GLO')], -self.number_of_cars :] =\
            (self.temp_array[self.array_inputs['battery cell mass'],:] * (1 + self.temp_array[self.array_inputs['fuel cell lifetime replacements'],:]))\
                / self.temp_array[self.array_inputs['lifetime kilometers'],:] *-1
        ...
        index_A = [self.inputs[c] for c in self.inputs\
                                                if any(ele in c[0] for ele in ['ICEV-d','ICEV-p', 'HEV-p', 'PHEV'])]
        index = self.get_index_from_array(['ICEV-d','ICEV-p', 'HEV-p', 'PHEV'])

        self.A[self.inputs[('polyethylene production, high density, granulate','RER')], index_A] = \
            self.temp_array[self.array_inputs['fuel tank mass'], index]\
                / self.temp_array[self.array_inputs['lifetime kilometers'], index] * -1
        ...
        self.A[self.inputs[("Hydrogen, gaseous, 700 bar, from electrolysis, at H2 fuelling station",'RER')], self.index_fuel_cell] = \
            self.temp_array[self.array_inputs['fuel mass'], index]\
                / self.temp_array[self.array_inputs['range'], index] * -1
        ...
        self.A[self.inputs[("Carbon dioxide, fossil", ('air',))], -self.number_of_cars :] =\
            (self.temp_array[self.array_inputs['CO2 per kg fuel'],:] * self.temp_array[self.array_inputs['fuel mass'],:])\
                / self.temp_array[self.array_inputs['range'],:] *-1
        ...
        self.A[self.index_emissions, -self.number_of_cars :] = self.temp_array[[self.array_inputs[l] for l in list_direct_emissions],:] *-1

        # Noise emissions
        self.A[self.index_noise, -self.number_of_cars :] = self.temp_array[[self.array_inputs[self.rev_inputs[e][0]] for e in self.index_noise],:] *-1
