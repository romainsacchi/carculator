...
class VehicleModel:
    ...
    def set_electricity_consumption(self) -> None:
        ...
        self["fuel consumption"] = (
            self["fuel mass"] / self[var] / _(self["fuel density per kg"])
        )
    ...
    def create_PHEV(self):
        ...
        for pwt, pwtc in (("PHEV-d", "PHEV-c-d"), ("PHEV-p", "PHEV-c-p")):
            if pwt in self.array.coords["powertrain"].values:
                ...
                self.array.loc[
                    dict(parameter="electricity consumption", powertrain=pwt)
                ] = (
                    self.array.loc[
                        dict(parameter="electric energy stored", powertrain=pwt)
                    ]
                ...
                self.array.loc[dict(parameter="fuel consumption", powertrain=pwt)] = (
                    self.array.loc[dict(parameter="fuel mass", powertrain=pwt)]
                    / self.array.loc[dict(parameter=var, powertrain=pwt)]
                    / self.array.loc[
                        dict(parameter="fuel density per kg", powertrain=pwt)
                    ]
                )
    ...
    def set_hot_emissions(self) -> None:
        ...
        self.array.loc[dict(parameter=list_direct_emissions,)] = hem.get_hot_emissions(
